// Dachi_Trader_v13_11_43.mq5 — Expert Advisor
// Version: 13.11.43
// Clean Core branch: simplified runtime pipeline (F2 + HTF gate).

#property copyright   "Daniel @danieljulyanto"
#property version     "13.11.43"
#property description "Dachi Trader v13.11.43 — Expert Advisor"
...
#define DB_CLR_CREAM      C'245,222,179'  // v13.11.43: RE-ENTRY label color
...
bool LIC_VerifyOnce(){
    ...
    string body = StringFormat("{\"account_number\":\"%I64u\",\"ea_version\":\"13.11.43\"}", (ulong)account);
...
int CleanupAllEAObjects(){
    // v13.11.43: delete EA objects by prefix + object type on the main chart.
...
int OnInit(){
    ...
    // v13.11.43 — startup self-clean catches remnants from a prior abnormal
    ...
    Print("==============================================================");
    Print("[INIT] Dachi Trader v13.11.43 | ",_Symbol," ",EnumToString(_Period),
...
void OnDeinit(const int reason){
    ...
    Print("[DEINIT] Dachi v13.11.43 removed reason=",reason," cleanup_deleted=",deleted);
...
string JournalHeader(){
    return "event_time;symbol;timeframe;origin;signal_class;direction;signal_entry_price;signal_exit_time;signal_exit_price;signal_move_pts;signal_duration_bars;signal_result;signal_exit_reason;executed;skip_reason;block_reason;bre_reason;atr;spread;ema_fast;slow_ma;ma_gap_pts;slow_angle_deg;vline_state;vline_dir;sw_state;sw_score;adx;di_plus;di_minus;sl;tp1;tp2;tp3;tp4;tp5";
}
...
void JournalFlushPending(datetime exit_time, double exit_price){
    ...
    string line=g_journal_pending_prefix+";"+JTime(exit_time)+";"+(exit_price>0.0?JNum(exit_price,_Digits):"")+";"+
        (exit_time>0?JNum(move_pts,1):"")+";"+(exit_time>0?IntegerToString(duration_bars):"")+";"+result+";"+exit_reason+";"+g_journal_pending_suffix;
...
int ComputeFilterHash(){
    ...
    h = (h ^ (uint)0x13C00430)             * 16777619;   // logic version marker
    return (int)h;
}
...
void DrawSignalLabel(int dir,datetime t,double price,ENUM_SIGNAL_CLASS sc){
    ...
    if(ObjectFind(0,nm)>=0){
        ...
        // v13.11.43 — do not let stale BLOCKED labels survive a settings change.
