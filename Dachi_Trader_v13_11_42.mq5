// Dachi_Trader_v13_11_42.mq5 — Expert Advisor
// Version: 13.11.42
// Clean Core branch: simplified runtime pipeline (F2 + HTF gate).

#property copyright   "Daniel @danieljulyanto"
#property version     "13.11.42"
#property description "Dachi Trader v13.11.42 — Expert Advisor"
...
#define DB_CLR_CREAM      C'245,222,179'  // v13.11.42: RE-ENTRY label color
...
bool LIC_VerifyOnce(){
    ...
    string body = StringFormat("{\"account_number\":\"%I64u\",\"ea_version\":\"13.11.42\"}", (ulong)account);
...
int CleanupAllEAObjects(){
    // v13.11.42: delete EA objects by prefix + object type on the main chart.
...
int OnInit(){
    ...
    // v13.11.42 — startup self-clean catches remnants from a prior abnormal
    ...
    Print("==============================================================");
    Print("[INIT] Dachi Trader v13.11.42 | ",_Symbol," ",EnumToString(_Period),
...
void OnDeinit(const int reason){
    ...
    Print("[DEINIT] Dachi v13.11.42 removed reason=",reason," cleanup_deleted=",deleted);
...
string JournalSignalPath(){
    ...
    StringReplace(d,".","");
    return folder+"\\Dachi_Signal_Journal_"+_Symbol+"_"+tf+"_"+d+".csv";
}
...
void JournalEnsureFile(){
    ...
    FileFlush(h);
    FileClose(h);
    Print("[JOURNAL] ready path=",path," common=",(InpJournalUseCommonFolder?"YES":"NO"));
}
...
int ComputeFilterHash(){
    ...
    h = (h ^ (uint)0x13C00420)             * 16777619;   // logic version marker
    return (int)h;
}
...
void DrawSignalLabel(int dir,datetime t,double price,ENUM_SIGNAL_CLASS sc){
    ...
    if(ObjectFind(0,nm)>=0){
        ...
        // v13.11.42 — do not let stale BLOCKED labels survive a settings change.
