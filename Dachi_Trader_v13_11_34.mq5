// Dachi_Trader_v13_11_34.mq5 — Expert Advisor
// Version: 13.11.34
// Clean Core branch: simplified runtime pipeline (F2 + HTF gate).

#property copyright   "Daniel @danieljulyanto"
#property version     "13.11.34"
#property description "Dachi Trader v13.11.34 — Expert Advisor"
...
bool LIC_VerifyOnce(){
    ...
    string body = StringFormat("{\"account_number\":\"%I64u\",\"ea_version\":\"13.11.34\"}", (ulong)account);
...
int CleanupAllEAObjects(){
    // Fast and synchronous cleanup. ObjectsDeleteAll(prefix) removes every
    // object whose name starts with the prefix, so one DT13 pass covers current
    // DT13_ objects and older DT13-prefixed objects without thousands of async
    // ObjectDelete() calls that can trigger MT5 abnormal termination on remove.
    ResetLastError();
    int deleted=ObjectsDeleteAll(0,"DT13",-1,-1);
    int err=GetLastError();
    if(err!=0) Print("[DEINIT-CLEANUP] ObjectsDeleteAll DT13 err=",err," deleted=",deleted);
    LIC_ClearOverlay();
    return deleted;
}
...
int OnInit(){
    ...
    // v13.11.34 — startup self-clean catches remnants from a prior abnormal
    ...
    Print("==============================================================");
    Print("[INIT] Dachi Trader v13.11.34 | ",_Symbol," ",EnumToString(_Period),
...
void OnDeinit(const int reason){
    ...
    ChartRedraw();
    Print("[DEINIT] Dachi v13.11.34 removed reason=",reason," cleanup_deleted=",deleted);
}
...
uint ComputeFilterHash(){
    ...
    h = (h ^ (uint)0x13C00340)             * 16777619;   // logic version marker
    return h;
}
...
void DrawSignalLabel(int dir,datetime t,double price,ENUM_SIGNAL_CLASS sc){
    ...
    if(ObjectFind(0,nm)>=0){
        ...
        // v13.11.34 — do not let stale BLOCKED labels survive a settings change.
