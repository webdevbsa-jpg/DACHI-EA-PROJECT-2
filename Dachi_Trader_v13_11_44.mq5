// Dachi_Trader_v13_11_44.mq5 — Expert Advisor
// Version: 13.11.44
// Clean Core branch: simplified runtime pipeline (F2 + HTF gate).

#property copyright   "Daniel @danieljulyanto"
#property version     "13.11.44"
#property description "Dachi Trader v13.11.44 — Expert Advisor"
...
bool LIC_VerifyOnce(){
    ...
    string body = StringFormat("{\"account_number\":\"%I64u\",\"ea_version\":\"13.11.44\"}", (ulong)account);
...
int OnInit(){
    ...
    Print("==============================================================");
    Print("[INIT] Dachi Trader v13.11.44 | ",_Symbol," ",EnumToString(_Period),
...
void OnDeinit(const int reason){
    ...
    Print("[DEINIT] Dachi v13.11.44 removed reason=",reason," cleanup_deleted=",deleted);
...
void JournalSignalEvent(int dir, datetime t, double price, ENUM_SIGNAL_CLASS sc, string block_reason, string bre_reason, bool executed, string skip_reason, string origin){
    ...
    double risk=atr*(limited?InpSoft_SL_Mult:InpSL_Mult);
    double p1=atr*(limited?InpSoft_TP1_Mult:InpTP1_Mult);
    double p2=p1+atr*(limited?InpSoft_TP2_Mult:InpTP1_to_TP2_Mult);
    sl=(dir==1?price-risk:price+risk);
    tp1=(dir==1?price+p1:price-p1);
    tp2=(dir==1?price+p2:price-p2);
    tp3=(dir==1?price+3*p1:price-3*p1);
    tp4=(dir==1?price+4*p1:price-4*p1);
    tp5=(dir==1?price+5*p1:price-5*p1);
...
int ComputeFilterHash(){
    ...
    h = (h ^ (uint)0x13C00440)             * 16777619;   // logic version marker
    return (int)h;
}
...
bool EvaluatePipeline(int sig){
    ...
    g_pipe_filter_hard = h_f2 || h_htf || h_est || h_slow_angle || h_sw || h_sideway || h_atr_block || h_sq;
    g_pipe_soft = s_sw || s_atr_limited || s_sq;
    ...
    for(int i=scan;i>=1;i--){
        ...
        if(hard){
            ...
            if(FindBlockedRetestReentryAt(i,sig,bre_reason,bre_t,bre_p)){
                ...
                double bre_risk=ah[0]*(bre_limited?InpSoft_SL_Mult:InpSL_Mult);
                vis_sl=(sig==1)?bre_p-bre_risk:bre_p+bre_risk;
                double bre_tp1=ah[0]*(bre_limited?InpSoft_TP1_Mult:InpTP1_Mult);
                double bre_tp2=bre_tp1+ah[0]*(bre_limited?InpSoft_TP2_Mult:InpTP1_to_TP2_Mult);
                vis_tp[0]=(sig==1)?bre_p+bre_tp1:bre_p-bre_tp1;
                vis_tp[1]=(sig==1)?bre_p+bre_tp2:bre_p-bre_tp2;
...
void DrawDashboard(bool force){
    ...
    int nr=30,ph=(nr+1)*DB_LINE_H+DB_Y_BASE+8;
