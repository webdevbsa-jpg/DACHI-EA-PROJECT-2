// Dachi_Trader_v13_11_22.mq5 — Expert Advisor
// Version: 13.11.22
// Clean Core branch: simplified runtime pipeline (F2 + HTF gate).

#property copyright   "Daniel @danieljulyanto"
#property version     "13.11.22"
#property description "Dachi Trader v13.11.22 — Expert Advisor"
...
bool LIC_VerifyOnce(){
    ...
    string body = StringFormat("{\"account_number\":\"%I64u\",\"ea_version\":\"13.11.22\"}", (ulong)account);
...
int OnInit(){
    ...
    Print("==============================================================");
    Print("[INIT] Dachi Trader v13.11.22 | ",_Symbol," ",EnumToString(_Period),
...
void OnDeinit(const int reason){
    ...
    LIC_ClearOverlay();
    Print("[DEINIT] Dachi v13.11.22 removed");
}
...
uint ComputeFilterHash(){
    ...
    h = (h ^ (uint)0x13C00220)             * 16777619;   // logic version marker
    return h;
}
...
bool BuildEvasiveSTSeries(ENUM_TIMEFRAMES tf, int bars, double &bands[], int &dirs[], bool &noisies[], bool &changed[]){
    ...
    int warmup=MathMax(80, period*10);
    ...
    for(int k=need-1; k>=0; k--){
        ...
        prev_trend=trend;
        noisy=(MathAbs(rates[k].close-prev_band) < (atr[k]*InpEST_NoiseThresholdATR));

        if(trend==1){
            st_band = noisy ? (prev_band - atr[k]*InpEST_ExpansionAlphaATR) : MathMax(lower_base, prev_band);
            if(rates[k].close < st_band){ trend=-1; st_band=upper_base; }
        } else {
            st_band = noisy ? (prev_band + atr[k]*InpEST_ExpansionAlphaATR) : MathMin(upper_base, prev_band);
...
void DrawEvasiveSTVisual(){
    ...
    ENUM_TIMEFRAMES tf=(InpEST_TF==PERIOD_CURRENT?_Period:InpEST_TF);
    int bars=MathMax(5,MathMin(InpEST_VisualBars,300));
    ...
    for(int k=bars; k>=1; k--){
        ...
        if(InpEST_ShowSwitchLabels && changed[k]){
            ...
            ObjectSetInteger(0,nl,OBJPROP_BGCOLOR,(dirs[k]>0?C'8,153,129':C'242,54,69'));
