// Dachi_Trader_v13_11_21.mq5 — Expert Advisor
// Version: 13.11.21
// Clean Core branch: simplified runtime pipeline (F2 + HTF gate).

#property copyright   "Daniel @danieljulyanto"
#property version     "13.11.21"
#property description "Dachi Trader v13.11.21 — Expert Advisor"
...
input group "=== Evasive Supertrend Bias Guard ==="
input bool            InpUseEvasiveSTGuard      = false;
input ENUM_TIMEFRAMES InpEST_TF                 = PERIOD_M15;
input int             InpEST_ATRLength          = 10;
input double          InpEST_BaseMultiplier     = 3.0;
input double          InpEST_NoiseThresholdATR  = 1.0;
input double          InpEST_ExpansionAlphaATR  = 0.5;
input bool            InpEST_BlockNeutral       = false;
input bool            InpEST_ShowVisual         = true;
input bool            InpEST_ShowSwitchLabels   = true;
input int             InpEST_VisualBars         = 80;
...
bool LIC_VerifyOnce(){
    ...
    string body = StringFormat("{\"account_number\":\"%I64u\",\"ea_version\":\"13.11.21\"}", (ulong)account);
...
int OnInit(){
    ...
    Print("==============================================================");
    Print("[INIT] Dachi Trader v13.11.21 | ",_Symbol," ",EnumToString(_Period),
...
void OnDeinit(const int reason){
    ...
    LIC_ClearOverlay();
    Print("[DEINIT] Dachi v13.11.21 removed");
}
...
uint ComputeFilterHash(){
    ...
    h = (h ^ (uint)(InpUseEvasiveSTGuard?1:0)) * 16777619;
    h = (h ^ (uint)InpEST_TF) * 16777619;
    h = (h ^ (uint)InpEST_ATRLength) * 16777619;
    h = (h ^ (uint)(int)(InpEST_BaseMultiplier*100)) * 16777619;
    h = (h ^ (uint)(int)(InpEST_NoiseThresholdATR*100)) * 16777619;
    h = (h ^ (uint)(int)(InpEST_ExpansionAlphaATR*100)) * 16777619;
    h = (h ^ (uint)(InpEST_BlockNeutral?1:0)) * 16777619;
    ...
    h = (h ^ (uint)0x13C00210)             * 16777619;   // logic version marker
    return h;
}
...
// Evasive SuperTrend logic ported from user-provided PineScript (© LuxAlgo, CC BY-NC-SA 4.0).
...
bool CalcEvasiveSupertrendAt(ENUM_TIMEFRAMES tf, int idx, double &out_band, int &out_dir, bool &out_noisy, bool &out_changed){
    ...
    int period=MathMax(1, InpEST_ATRLength);
    int warmup=MathMax(60, period*8);
    ...
    if(CopyBuffer(hatr,0,0,need,atr)<need){ IndicatorRelease(hatr); return false; }
    IndicatorRelease(hatr);

    double st_band=0.0;
    bool has_band=false;
    int trend=1;
    int prev_trend_for_target=1;
    bool noisy=false;
    ...
    for(int k=idx+warmup; k>=idx; k--){
        ...
        double src=(hi+lo)*0.5;
        double upper_base=src + InpEST_BaseMultiplier*atr[k];
        double lower_base=src - InpEST_BaseMultiplier*atr[k];
        double prev_band=has_band ? st_band : (trend==1 ? lower_base : upper_base);
        noisy=(MathAbs(cl-prev_band) < (atr[k]*InpEST_NoiseThresholdATR));

        if(trend==1){
            if(noisy) st_band = prev_band - (atr[k]*InpEST_ExpansionAlphaATR);
            else      st_band = MathMax(lower_base, prev_band);
            if(cl < st_band){ trend=-1; st_band=upper_base; }
        } else {
            if(noisy) st_band = prev_band + (atr[k]*InpEST_ExpansionAlphaATR);
            else      st_band = MathMin(upper_base, prev_band);
            if(cl > st_band){ trend=1; st_band=lower_base; }
        }
        has_band=true;
        if(k==idx+1) prev_trend_for_target=trend;
    }

    out_band=st_band;
    out_dir=trend;
    out_noisy=noisy;
    out_changed=(trend!=prev_trend_for_target);
    return true;
}

bool EvasiveSTBlocks(int sig, int idx){
    g_est_dir=0; g_est_band=0.0; g_est_noisy=false; g_est_state=InpUseEvasiveSTGuard?"ERR":"OFF";
    if(!InpUseEvasiveSTGuard) return false;
    ENUM_TIMEFRAMES tf=(InpEST_TF==PERIOD_CURRENT?_Period:InpEST_TF);
    int tf_shift=idx;
    if(tf!=_Period){
        datetime t=iTime(_Symbol,_Period,idx);
        tf_shift=iBarShift(_Symbol,tf,t,false);
        if(tf_shift<0){ g_est_state="NO_BAR"; return InpEST_BlockNeutral; }
    }
    bool changed=false;
    if(!CalcEvasiveSupertrendAt(tf,tf_shift,g_est_band,g_est_dir,g_est_noisy,changed)){
        g_est_state="NO_DATA";
        return InpEST_BlockNeutral;
    }
...
void DrawEvasiveSTVisual(){
    ClearEvasiveSTVisual();
    if(!InpEST_ShowVisual) return;
    ENUM_TIMEFRAMES tf=(InpEST_TF==PERIOD_CURRENT?_Period:InpEST_TF);
    int bars=MathMax(5,MathMin(InpEST_VisualBars,200));
    if(Bars(_Symbol,tf)<bars+InpEST_ATRLength+20) return;
    for(int k=bars; k>=1; k--){
        double band=0.0; int dir=0; bool noisy=false, changed=false;
        if(!CalcEvasiveSupertrendAt(tf,k,band,dir,noisy,changed)) continue;
        datetime t0=iTime(_Symbol,tf,k), t1=iTime(_Symbol,tf,k-1);
        string n=ObjName("EST_LINE_"+IntegerToString(k));
        if(ObjectFind(0,n)<0) ObjectCreate(0,n,OBJ_TREND,0,t0,band,t1,band);
        else{ ObjectSetInteger(0,n,OBJPROP_TIME,0,t0); ObjectSetDouble(0,n,OBJPROP_PRICE,0,band); ObjectSetInteger(0,n,OBJPROP_TIME,1,t1); ObjectSetDouble(0,n,OBJPROP_PRICE,1,band); }
        ObjectSetInteger(0,n,OBJPROP_COLOR,(dir>0?C'8,153,129':C'242,54,69'));
        ObjectSetInteger(0,n,OBJPROP_WIDTH,2);
        ObjectSetInteger(0,n,OBJPROP_STYLE,noisy?STYLE_DOT:STYLE_SOLID);
        ...
        if(InpEST_ShowSwitchLabels && changed){
            string nl=ObjName("EST_LBL_"+IntegerToString(k));
            if(ObjectFind(0,nl)<0) ObjectCreate(0,nl,OBJ_TEXT,0,t0,band);
            ObjectSetString(0,nl,OBJPROP_TEXT,(dir>0?"BULL":"BEAR"));
            ObjectSetInteger(0,nl,OBJPROP_COLOR,clrWhite);
            ObjectSetInteger(0,nl,OBJPROP_BGCOLOR,(dir>0?C'8,153,129':C'242,54,69'));
            ObjectSetInteger(0,nl,OBJPROP_FONTSIZE,8);
            ObjectSetInteger(0,nl,OBJPROP_ANCHOR,(dir>0?ANCHOR_LOWER:ANCHOR_UPPER));
...
ENUM_FILTER_DECISION EvalATRHealthAt(int idx, string &state){
    ...
    double atrPts = a14[0]/_Point;
    double fastRatio = a5[0]/a14[0];
    if(atrPts >= InpATR_M5_BlockAbove || fastRatio >= InpATR_FastRatioBlock){ state="CHAOS"; return DEC_BLOCKED; }
    if(atrPts < InpATR_M5_MinTrade){ state="LOWVOL"; return DEC_LIMITED; }
    if(atrPts > InpATR_M5_MaxAllowed){ state="TOO_HIGH"; return DEC_BLOCKED; }
    if(atrPts > InpATR_M5_MaxNormal || fastRatio >= InpATR_FastRatioLimited){ state="LIMITED"; return DEC_LIMITED; }
...
void DrawDashboard(bool force){
    ...
    bool est_block = EvasiveSTBlocks((g_last_signal==0?1:g_last_signal),1);
    if(htf_block) n_hard++;
    if(est_block) n_hard++;
    ...
    DrawDR("EST",row++,"Evasive ST", (InpUseEvasiveSTGuard?(g_est_state+" "+EnumToString(InpEST_TF)):"OFF"), !InpUseEvasiveSTGuard?DB_CLR_GRAY:(est_block?DB_CLR_RED:(g_est_dir>0?DB_CLR_GREEN:DB_CLR_ORANGE)), yb);
            ...
            if(!PreCheckMarketConditions()){
                ...
                if(pipe_ok&&!g_pipe_hard){
                    ...
                    if(g_last_signal==0){
                        ...
                        else if(EvasiveSTBlocks(sig,1)) breason="EVASIVE_ST";
