// Dachi_Trader_v13_11_39.mq5 — Expert Advisor
// Version: 13.11.39
// Clean Core branch: simplified runtime pipeline (F2 + HTF gate).

#property copyright   "Daniel @danieljulyanto"
#property version     "13.11.39"
#property description "Dachi Trader v13.11.39 — Expert Advisor"
...
#define DB_CLR_CREAM      C'245,222,179'  // v13.11.39: RE-ENTRY label color
...
bool LIC_VerifyOnce(){
    ...
    string body = StringFormat("{\"account_number\":\"%I64u\",\"ea_version\":\"13.11.39\"}", (ulong)account);
...
int CleanupAllEAObjects(){
    // v13.11.39: delete EA objects by prefix + object type on the main chart.
...
int OnInit(){
    ...
    // v13.11.39 — startup self-clean catches remnants from a prior abnormal
    ...
    Print("==============================================================");
    Print("[INIT] Dachi Trader v13.11.39 | ",_Symbol," ",EnumToString(_Period),
...
void OnDeinit(const int reason){
    ...
    Print("[DEINIT] Dachi v13.11.39 removed reason=",reason," cleanup_deleted=",deleted);
...
void JournalSignalEvent(int dir, datetime t, double price, ENUM_SIGNAL_CLASS sc, string block_reason, string bre_reason, bool executed, string skip_reason, string origin){
    ...
    double sw=0.0; bool sw_chop=CalcSWAt(idx,sw); string sw_state=(InpSW_Action==F_OFF?"OFF":(sw_chop?"CHOP":(g_sw_state==""?"ORDER":g_sw_state)));
...
int ComputeFilterHash(){
    ...
    h = (h ^ (uint)0x13C00390)             * 16777619;   // logic version marker
    return h;
}
...
bool EvaluatePipeline(int sig){
    ...
    g_pipe_filter_hard = h_f2 || h_htf || h_est || h_slow_angle || h_sw || h_sideway || h_atr || h_sq;
    g_pipe_soft = s_sw || s_atr || s_sq;
...
void DrawSignalLabel(int dir,datetime t,double price,ENUM_SIGNAL_CLASS sc){
    ...
    if(ObjectFind(0,nm)>=0){
        ...
        // v13.11.39 — do not let stale BLOCKED labels survive a settings change.
