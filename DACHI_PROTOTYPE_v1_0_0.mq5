// DACHI_PROTOTYPE_v1_0_0.mq5 — DACHI PROTOTYPE Expert Advisor
// Version: 1.00
// Prototype branch: M5 rulebook-aligned parallel build (M30/M15 context, ADX band, fixed risk).

#property copyright   "Daniel @danieljulyanto"
#property version     "1.00"
#property description "DACHI PROTOTYPE v1.00 — M5 Institutional Rulebook EA"
...
#define DB_CLR_CREAM      C'245,222,179'  // v13.11.44: RE-ENTRY label color
...
// DACHI PROTOTYPE v1.00 — rulebook profile selectors.
enum ENUM_RISK_PROFILE  { RISK_M5_BALANCED=0, RISK_M15_CONTEXT=1, RISK_M30_SWING=2, RISK_ATR_DYNAMIC=3 };

// === INPUTS ===
input group "=== EA Trading ==="
input string InpPrototypeName    = "DACHI PROTOTYPE";
...
input group "=== Backtest Auto Journal ==="
input bool   InpUseAutoJournal       = true;   // export signal/backtest journal CSV automatically
input bool   InpJournalOnlyTester    = true;   // only write journal in Strategy Tester/Optimizer/visual tester
input bool   InpJournalSignals       = true;   // record every signal: VALID/LIMITED/BLOCKED/RE-ENTRY/F3
input bool   InpJournalUseCommonFolder = true;  // tester-safe: write to Terminal/Common/Files when true
...
input bool   InpUseSlowMAAngleGuard   = false;
input double InpSlowMA_MinAngleDeg    = 0.0;    // BUY needs >= +deg, SELL needs <= -deg; 0 allows flat in signal direction

input group "=== SW / Sideway Clustering Filter ==="
input ENUM_FILTER_ACTION InpSW_Action         = F_SOFT; // OFF/SOFT/HARD anti-chop entropy gate
input int                InpSW_Lookback       = 18;     // rolling candle-structure window
input double             InpSW_ChopThreshold  = 0.82;   // high entropy needed before CHOP can trigger
...
input group "=== Rulebook HTF Context (M30 Bias + M15 Setup) ==="
input bool   InpUseM30Bias        = true;
...
input bool            InpUseVLineGuard      = false;  // rulebook default: visual/context only; hard entry guard OFF
...
input bool            InpUseVLineNoiseGuard       = true;   // V-Line noise => LIMITED/BLOCK context treatment
input ENUM_FILTER_ACTION InpVLineNoiseAction      = F_SOFT; // default LIMITED
...
input group "=== Blocked Signal Retest Re-entry ==="
input bool   InpUseBlockedRetestReentry   = true;   // professional re-entry after a good signal was hard-blocked
input int    InpBRE_Bars                  = 8;      // bars to wait for retest confirmation
input double InpBRE_RetestBufferATR       = 0.20;   // MA-band retest tolerance in ATR
input bool   InpBRE_RequireMAAlign        = true;   // BUY fast>slow, SELL fast<slow
input bool   InpBRE_RequireDIDirection    = true;   // BUY DI+>DI-, SELL DI->DI+
input double InpBRE_DIMargin              = 0.0;    // optional DI margin
input double InpBRE_MinADX                = 18.0;   // minimum ADX strength before re-entry can fire
...
input bool   InpBRE_EnterAsLimited        = true;   // true=BRE uses LIMITED/tight TP-SL, false=BRE uses VALID/normal TP-SL
...
input bool   InpBRE_AllowOtherBlocks      = false;  // fallback/unknown block reasons

// === RETIRED LEGACY RE-ENTRY CONSTANTS (hidden from Inputs) ===
...
input group "=== Rulebook Risk Profile ==="
input ENUM_RISK_PROFILE InpRiskProfile = RISK_M5_BALANCED;
input int    InpRisk_M5_SL_Points  = 900;
input int    InpRisk_M5_TP1_Points = 900;
input int    InpRisk_M5_TP2_Points = 1800;
input int    InpRisk_M5_TP3_Points = 2700;
input int    InpRisk_M5_TP4_Points = 3600;
input int    InpRisk_M5_TP5_Points = 5000;
input int    InpRisk_M15_SL_Points  = 1500;
input int    InpRisk_M15_TP1_Points = 1500;
input int    InpRisk_M15_TP2_Points = 3000;
input int    InpRisk_M15_TP3_Points = 4500;
input int    InpRisk_M15_TP4_Points = 6000;
input int    InpRisk_M15_TP5_Points = 9000;
input int    InpRisk_M30_SL_Points  = 2100;
input int    InpRisk_M30_TP1_Points = 2100;
input int    InpRisk_M30_TP2_Points = 4200;
input int    InpRisk_M30_TP3_Points = 6300;
input int    InpRisk_M30_TP4_Points = 8400;
input int    InpRisk_M30_TP5_Points = 12000;

input group "=== ADX Band Guard (M5 Rulebook) ==="
input bool   InpUseADXBandGuard = true;
input double InpADX_MinAllowed  = 15.0;
input double InpADX_MaxAllowed  = 30.0;
input double InpADX_ExtremeLevel = 40.0;
input ENUM_FILTER_ACTION InpADX_ExtremeAction = F_HARD;
...
bool g_bre_armed=false, g_bre_pullback_seen=false; int g_bre_dir=0, g_bre_counter=0;
datetime g_bre_time=0; double g_bre_atr=0.0; string g_bre_reason="OFF";
// SL state
...
// Backtest signal journal pending row; finalized when the next signal supplies the exit price.
bool g_journal_pending=false;
string g_journal_pending_prefix="";
string g_journal_pending_suffix="";
datetime g_journal_pending_time=0;
double g_journal_pending_entry=0.0;
int g_journal_pending_dir=0;
datetime g_journal_position_exit_time=0;
double g_journal_position_exit_price=0.0;
string g_journal_position_exit_reason="";
string g_journal_session_id="";
...
double g_sw_score=0.0, g_sw_direction_balance=0.0, g_sw_body_dominance=0.0; bool g_sw_trig=false; string g_sw_state="OFF";
...
double g_adx_band_value = EMPTY_VALUE;
string g_adx_band_state = "OFF";
string g_adx_band_reason = "ADX_OFF";
string g_vline_noise_state = "OFF";
...
bool LIC_VerifyOnce(){
    ...
    string body = StringFormat("{\"account_number\":\"%I64u\",\"ea_version\":\"DACHI_PROTOTYPE_1.00\"}", (ulong)account);
...
int CleanupObjectsByPrefixAndType(string prefix, int object_type){
    ResetLastError();
    int deleted=ObjectsDeleteAll(0,prefix,0,object_type);
    int err=GetLastError();
    if(err!=0) Print("[EA-CLEANUP] ObjectsDeleteAll prefix=",prefix," type=",object_type," err=",err," deleted=",deleted);
    return (deleted>0 ? deleted : 0);
}

int CleanupAllEAObjects(){
    // v13.11.44: delete EA objects by prefix + object type on the main chart.
    ...
    int types[8];
    types[0]=OBJ_TEXT; types[1]=OBJ_LABEL; types[2]=OBJ_BUTTON; types[3]=OBJ_RECTANGLE;
    types[4]=OBJ_RECTANGLE_LABEL; types[5]=OBJ_TREND; types[6]=OBJ_TRIANGLE; types[7]=OBJ_HLINE;

    int total_deleted=0;
    for(int i=0;i<ArraySize(types);i++) total_deleted+=CleanupObjectsByPrefixAndType(OBJ_PREFIX,types[i]);
    total_deleted+=CleanupObjectsByPrefixAndType(OBJ_PREFIX,-1);

    // Legacy safety for any old object that used DT13 without underscore.
    for(int j=0;j<ArraySize(types);j++) total_deleted+=CleanupObjectsByPrefixAndType("DT13",types[j]);
    total_deleted+=CleanupObjectsByPrefixAndType("DT13",-1);
...
void ResetRuntimeTradeState(bool clear_visual=true){
    ...
    g_bre_armed=false;g_bre_pullback_seen=false;g_bre_dir=0;g_bre_counter=0;g_bre_time=0;g_bre_atr=0.0;g_bre_reason="OFF";
...
void BuildFallbackTradeFromPosition(int dir){
    ...
    CalcTradeLevels(dir,open_price,atr,g_soft_mode,g_sl_price,g_tp);
    g_sl_initial_dist=MathAbs(open_price-g_sl_price);
...
int OnInit(){
    ...
    g_pipe_hard=false;g_pipe_soft=false;g_last_decision="---";g_last_block_reason="---";
    g_journal_pending=false;g_journal_pending_prefix="";g_journal_pending_suffix="";g_journal_pending_time=0;g_journal_pending_entry=0.0;g_journal_pending_dir=0;
    g_journal_position_exit_time=0;g_journal_position_exit_price=0.0;g_journal_position_exit_reason="";
    g_journal_session_id="";
    JournalEnsureFile();
    g_last_signal_gap_pts=0;g_slowma_angle_deg=0.0;g_slowma_angle_block=false;g_slowma_angle_state="OFF";
    g_sw_score=0.0;g_sw_direction_balance=0.0;g_sw_body_dominance=0.0;g_sw_trig=false;g_sw_state="OFF";
    ...
    // v13.11.44 — startup self-clean catches remnants from a prior abnormal
    ...
    if(InpF1_Action!=F_OFF || InpRegime_Enable || InpUseBlockedRetestReentry || InpSW_UseTrendOverride || InpUseADXBandGuard){
    ...
    Print("==============================================================");
    Print("[INIT] ",InpPrototypeName," v1.00 | ",_Symbol," ",EnumToString(_Period),
    ...
    Print("[INIT] Rulebook: Risk=",EnumToString(InpRiskProfile)," | ADXBand=",(InpUseADXBandGuard?"ON":"OFF"),
          " min=",DoubleToString(InpADX_MinAllowed,1)," max=",DoubleToString(InpADX_MaxAllowed,1),
          " | VLineHard=",(InpUseVLineGuard?"ON":"OFF")," VLineNoise=",(InpUseVLineNoiseGuard?EnumToString(InpVLineNoiseAction):"OFF"));
...
void OnDeinit(const int reason){
    EventKillTimer();
    JournalFlushPending(0,0.0);
    int deleted=CleanupAllEAObjects();
    Print("[DEINIT] ",InpPrototypeName," v1.00 removed reason=",reason," cleanup_deleted=",deleted);
...
// === BACKTEST AUTO JOURNAL ===
bool JournalEnabled(){
    if(!InpUseAutoJournal || !InpJournalSignals) return false;
    if(InpJournalOnlyTester && !MQLInfoInteger(MQL_TESTER)) return false;
    return true;
}

string JClean(string s){
    StringReplace(s,";","|");
    StringReplace(s,"\r"," ");
    StringReplace(s,"\n"," ");
    return s;
...
string JTime(datetime t){
    if(t<=0) return "";
    return TimeToString(t,TIME_DATE|TIME_MINUTES|TIME_SECONDS);
}

string JNum(double v, int digits=5){
    if(v==EMPTY_VALUE) return "";
    return DoubleToString(v,digits);
}

string SignalClassName(ENUM_SIGNAL_CLASS sc){
    switch(sc){
        case SC_VALID: return "VALID";
        case SC_SOFT: return "LIMITED";
        case SC_BLOCKED: return "BLOCKED";
        case SC_REENTRY: return "RE-ENTRY";
        case SC_REVERSAL: return "REVERSAL";
        case SC_F3_RECOVERY: return "F3_RECOVERY";
    }
    return "UNKNOWN";
}

string DirName(int dir){ return dir==1?"BUY":(dir==-1?"SELL":"NONE"); }
...
string JournalTimestampTag(datetime t){
    string d=TimeToString(t,TIME_DATE|TIME_MINUTES|TIME_SECONDS);
    StringReplace(d,".","");
    StringReplace(d,":","");
    StringReplace(d," ","_");
    return d;
...
string JournalSessionId(){
    if(g_journal_session_id==""){
        string local_tag=JournalTimestampTag(TimeLocal());
        g_journal_session_id=_Symbol+"_"+JournalTFTag()+"_"+local_tag+"_"+IntegerToString((int)GetTickCount());
    }
    return g_journal_session_id;
}

string JournalRootHint(){
    if(InpJournalUseCommonFolder)
        return TerminalInfoString(TERMINAL_COMMONDATA_PATH)+"\\Files";
    return TerminalInfoString(TERMINAL_DATA_PATH)+"\\MQL5\\Files";
}

string JournalSignalPath(){
    string folder="Dachi_Trader_Logs";
    FolderCreate(folder, InpJournalUseCommonFolder?FILE_COMMON:0);
    return folder+"\\Dachi_Signal_Journal_"+JournalSessionId()+".csv";
}

string JournalHeader(){
    return "event_time;symbol;timeframe;origin;signal_class;direction;signal_entry_price;signal_exit_time;signal_exit_price;signal_move_pts;signal_duration_bars;signal_result;signal_exit_reason;position_exit_time;position_exit_price;position_move_pts;position_duration_bars;position_result;position_exit_reason;executed;skip_reason;block_reason;bre_reason;atr;spread;ema_fast;slow_ma;ma_gap_pts;slow_angle_deg;vline_state;vline_dir;sw_state;sw_score;adx;di_plus;di_minus;sl;tp1;tp2;tp3;tp4;tp5";
}

bool JournalOpenFile(int &h, string &path){
    if(!JournalEnabled()) return false;
    path=JournalSignalPath();
    int flags=FILE_READ|FILE_WRITE|FILE_TXT|FILE_ANSI;
    if(InpJournalUseCommonFolder) flags|=FILE_COMMON;
    h=FileOpen(path,flags);
    if(h==INVALID_HANDLE){ Print("[JOURNAL] FileOpen failed: ",path," common=",(InpJournalUseCommonFolder?"YES":"NO")," err=",GetLastError()); return false; }
    bool need_header=(FileSize(h)==0);
    FileSeek(h,0,SEEK_END);
    if(need_header) FileWriteString(h,JournalHeader()+"\r\n");
    return true;
}

void JournalEnsureFile(){
    int h=INVALID_HANDLE; string path="";
    if(!JournalOpenFile(h,path)) return;
    FileFlush(h);
    FileClose(h);
    Print("[JOURNAL] ready path=",path," root=",JournalRootHint()," common=",(InpJournalUseCommonFolder?"YES":"NO")," session=",JournalSessionId());
}

void JournalAppendLine(string line){
    int h=INVALID_HANDLE; string path="";
    if(!JournalOpenFile(h,path)) return;
...
void JournalFlushPending(datetime exit_time, double exit_price){
    if(!g_journal_pending) return;
    double move_pts=0.0;
    int duration_bars=0;
    string result="OPEN";
    string exit_reason="TEST_END";
    if(exit_time>0 && exit_price>0.0){
        move_pts=(g_journal_pending_dir==1 ? (exit_price-g_journal_pending_entry) : (g_journal_pending_entry-exit_price))/_Point;
        if(move_pts>0.1) result="WIN";
        else if(move_pts<-0.1) result="LOSS";
        else result="BE";
        int entry_idx=iBarShift(_Symbol,_Period,g_journal_pending_time,false);
        int exit_idx =iBarShift(_Symbol,_Period,exit_time,false);
        if(entry_idx>=0 && exit_idx>=0) duration_bars=MathMax(0,entry_idx-exit_idx);
        exit_reason="NEXT_SIGNAL";
    }

    double pos_move_pts=0.0;
    int pos_duration_bars=0;
    string pos_result="";
    if(g_journal_position_exit_time>0 && g_journal_position_exit_price>0.0){
        pos_move_pts=(g_journal_pending_dir==1 ? (g_journal_position_exit_price-g_journal_pending_entry) : (g_journal_pending_entry-g_journal_position_exit_price))/_Point;
        if(pos_move_pts>0.1) pos_result="WIN";
        else if(pos_move_pts<-0.1) pos_result="LOSS";
        else pos_result="BE";
        int p_entry_idx=iBarShift(_Symbol,_Period,g_journal_pending_time,false);
        int p_exit_idx =iBarShift(_Symbol,_Period,g_journal_position_exit_time,false);
        if(p_entry_idx>=0 && p_exit_idx>=0) pos_duration_bars=MathMax(0,p_entry_idx-p_exit_idx);
    }

    string pos_fields=JTime(g_journal_position_exit_time)+";"+
        (g_journal_position_exit_price>0.0?JNum(g_journal_position_exit_price,_Digits):"")+";"+
        (g_journal_position_exit_time>0?JNum(pos_move_pts,1):"")+";"+
        (g_journal_position_exit_time>0?IntegerToString(pos_duration_bars):"")+";"+
        pos_result+";"+JClean(g_journal_position_exit_reason);

    string line=g_journal_pending_prefix+";"+JTime(exit_time)+";"+(exit_price>0.0?JNum(exit_price,_Digits):"")+";"+
        (exit_time>0?JNum(move_pts,1):"")+";"+(exit_time>0?IntegerToString(duration_bars):"")+";"+result+";"+exit_reason+";"+pos_fields+";"+g_journal_pending_suffix;
    JournalAppendLine(line);
    g_journal_pending=false;
    g_journal_pending_prefix="";
    g_journal_pending_suffix="";
    g_journal_pending_time=0;
    g_journal_pending_entry=0.0;
    g_journal_pending_dir=0;
    g_journal_position_exit_time=0;
    g_journal_position_exit_price=0.0;
    g_journal_position_exit_reason="";
}

void JournalMarkPositionExit(string reason, double price){
    if(!JournalEnabled() || !g_journal_pending) return;
    if(g_journal_position_exit_time>0) return;
    if(price<=0.0) price=(g_journal_pending_dir==1?SymbolInfoDouble(_Symbol,SYMBOL_BID):SymbolInfoDouble(_Symbol,SYMBOL_ASK));
    g_journal_position_exit_time=TimeCurrent();
    g_journal_position_exit_price=price;
    g_journal_position_exit_reason=reason;
}

void JournalSignalEvent(int dir, datetime t, double price, ENUM_SIGNAL_CLASS sc, string block_reason, string bre_reason, bool executed, string skip_reason, string origin){
    if(!JournalEnabled() || dir==0 || t<=0 || price<=0.0) return;

    JournalFlushPending(t,price);

    int idx=iBarShift(_Symbol,_Period,t,false);
    if(idx<0) idx=1;

    double atr=0.0, ema=0.0, slow=0.0, adx=EMPTY_VALUE, dip=EMPTY_VALUE, dim=EMPTY_VALUE, angle=EMPTY_VALUE;
    double b[1];
    if(CopyBuffer(h_atr,0,idx,1,b)>0) atr=b[0]; else atr=g_atr_val;
    if(CopyBuffer(h_ema_fast,0,idx,1,b)>0) ema=b[0];
    if(CopyBuffer(h_sma_slow,0,idx,1,b)>0) slow=b[0];
    CalcSlowMAVisualAngleAt(idx,angle);
    if(h_adx!=INVALID_HANDLE){
        if(CopyBuffer(h_adx,0,idx,1,b)>0) adx=b[0];
        if(CopyBuffer(h_adx,1,idx,1,b)>0) dip=b[0];
        if(CopyBuffer(h_adx,2,idx,1,b)>0) dim=b[0];
    }

    double vband=0.0; bool noisy=false; int vdir=0; string vstate="NO_DATA";
    GetVLineDirectionAt(idx,vdir,vstate,vband,noisy);
    double sw=0.0; string sw_calc_state="OFF"; bool sw_chop=CalcSWAt(idx,sw,sw_calc_state); string sw_state=(InpSW_Action==F_OFF?"OFF":sw_calc_state);

    double sl=0.0,tp1=0.0,tp2=0.0,tp3=0.0,tp4=0.0,tp5=0.0;
    bool limited=(sc==SC_SOFT || (sc==SC_REENTRY && InpBRE_EnterAsLimited));
    double journal_tp[MAX_TP];
    CalcTradeLevels(dir,price,atr,limited,sl,journal_tp);
    tp1=journal_tp[0];
    tp2=journal_tp[1];
    tp3=journal_tp[2];
    tp4=journal_tp[3];
    ...
    g_journal_pending_prefix=JTime(t)+";"+JClean(_Symbol)+";"+JClean(EnumToString(_Period))+";"+JClean(origin)+";"+
        SignalClassName(sc)+";"+DirName(dir)+";"+JNum(price,_Digits);
    g_journal_pending_suffix=(executed?"YES":"NO")+StringFormat(";%s;%s;%s;",JClean(skip_reason),JClean(block_reason),JClean(bre_reason))+
        JNum(atr,_Digits)+";"+IntegerToString((int)SymbolInfoInteger(_Symbol,SYMBOL_SPREAD))+";"+JNum(ema,_Digits)+";"+JNum(slow,_Digits)+";"+
        JNum((ema-slow)/_Point,1)+";"+JNum(angle,1)+";"+JClean(vstate)+";"+DirName(vdir)+";"+JClean(sw_state)+";"+JNum(sw,2)+";"+
        JNum(adx,2)+";"+JNum(dip,2)+";"+JNum(dim,2)+";"+
        JNum(sl,_Digits)+";"+JNum(tp1,_Digits)+";"+JNum(tp2,_Digits)+";"+JNum(tp3,_Digits)+";"+JNum(tp4,_Digits)+";"+JNum(tp5,_Digits);
    g_journal_position_exit_time=0;
    g_journal_position_exit_price=0.0;
    g_journal_position_exit_reason="";
    g_journal_pending=true;
    g_journal_pending_time=t;
    g_journal_pending_entry=price;
    g_journal_pending_dir=dir;
}

int ComputeFilterHash(){
    ...
    h = (h ^ (uint)InpSW_Action)          * 16777619;
    h = (h ^ (uint)InpSW_Lookback)        * 16777619;
    h = (h ^ (uint)(int)(InpSW_ChopThreshold*100)) * 16777619;
    ...
    h = (h ^ (uint)(InpBRE_RequireMAAlign?1:0)) * 16777619;
    h = (h ^ (uint)(InpBRE_RequireDIDirection?1:0)) * 16777619;
    h = (h ^ (uint)(int)(InpBRE_DIMargin*100)) * 16777619;
    h = (h ^ (uint)(int)(InpBRE_MinADX*100)) * 16777619;
    ...
    h = (h ^ (uint)(InpBRE_EnterAsLimited?1:0)) * 16777619;
    ...
    h = (h ^ (uint)(InpBRE_AllowOtherBlocks?1:0)) * 16777619;
    h = (h ^ (uint)InpF2_Action)           * 16777619;
    h = (h ^ (uint)(int)(InpF2_MaxDistATR*100))  * 16777619;

    h = (h ^ (uint)(InpUseM30Bias?1:0))          * 16777619;
    ...
    h = (h ^ (uint)(InpUseAutoJournal?1:0)) * 16777619;
    h = (h ^ (uint)(InpJournalOnlyTester?1:0)) * 16777619;
    h = (h ^ (uint)(InpJournalSignals?1:0)) * 16777619;
    h = (h ^ (uint)(InpJournalUseCommonFolder?1:0)) * 16777619;
    ...
    h = (h ^ (uint)InpRiskProfile) * 16777619;
    h = (h ^ (uint)InpRisk_M5_SL_Points) * 16777619;
    h = (h ^ (uint)InpRisk_M5_TP1_Points) * 16777619;
    h = (h ^ (uint)InpRisk_M5_TP2_Points) * 16777619;
    h = (h ^ (uint)InpRisk_M5_TP3_Points) * 16777619;
    h = (h ^ (uint)InpRisk_M5_TP4_Points) * 16777619;
    h = (h ^ (uint)InpRisk_M5_TP5_Points) * 16777619;
    h = (h ^ (uint)(InpUseADXBandGuard?1:0)) * 16777619;
    h = (h ^ (uint)(int)(InpADX_MinAllowed*100)) * 16777619;
    h = (h ^ (uint)(int)(InpADX_MaxAllowed*100)) * 16777619;
    h = (h ^ (uint)(int)(InpADX_ExtremeLevel*100)) * 16777619;
    h = (h ^ (uint)InpADX_ExtremeAction) * 16777619;
    h = (h ^ (uint)(InpUseVLineNoiseGuard?1:0)) * 16777619;
    h = (h ^ (uint)InpVLineNoiseAction) * 16777619;
    h = (h ^ (uint)0x10000000)             * 16777619;   // DACHI PROTOTYPE v1.00 logic version marker
    return (int)h;
}
...
void ProcessF3Recovery(bool new_bar){
    ...
    if(g_last_signal!=0){
        ...
        DrawTPSLLines();
        JournalSignalEvent(dir,recovery_bar_time,trigger_px,SC_F3_RECOVERY,g_f3_reason,"",true,"","F3");
    }
...
bool CalcSWAt(int idx, double &score, string &state){
    score=0.0; state="ERR";
    int look=MathMax(5, InpSW_Lookback);
    ...
    g_sw_direction_balance = MathAbs(bull_count - bear_count) / (double)total;
    g_sw_body_dominance = (double)(buckets[0] + buckets[1]) / (double)total;

    bool entropy_high = (score >= InpSW_ChopThreshold);
    bool directionless = (g_sw_direction_balance <= InpSW_DirectionalBalanceMax);
    bool body_weak = (g_sw_body_dominance <= InpSW_BodyDominanceMax);
    ...
    else if(score>=InpSW_MixedThreshold) state="MIXED "+DoubleToString(score,2);
...
bool SWTriggered(int idx){
    ...
    g_sw_trig=false;
    g_sw_state=(InpSW_Action==F_OFF?"OFF":"ERR");
...
bool HTFContextBlocks(int sig, int idx){
    bool blocked=false;
    if(InpUseM30Bias){
        int m30 = GetHTFDirection(PERIOD_M30, idx);
        if(m30==0 && !InpAllowNeutralBias) blocked=true;
        if(m30!=0 && sig!=m30) blocked=true;
    }
    if(InpUseM15Setup){
...
// === DACHI PROTOTYPE RULEBOOK HELPERS ===
bool UseFixedRiskProfile(bool tight_mode){
    return (!tight_mode && InpRiskProfile!=RISK_ATR_DYNAMIC);
}

int RiskProfileSLPoints(){
    switch(InpRiskProfile){
        case RISK_M15_CONTEXT: return InpRisk_M15_SL_Points;
        case RISK_M30_SWING:   return InpRisk_M30_SL_Points;
        case RISK_M5_BALANCED: return InpRisk_M5_SL_Points;
        default:               return 0;
    }
}

int RiskProfileTPPoints(int zero_based_index){
    int i=MathMax(0,MathMin(4,zero_based_index));
    if(InpRiskProfile==RISK_M15_CONTEXT){
        if(i==0) return InpRisk_M15_TP1_Points;
        if(i==1) return InpRisk_M15_TP2_Points;
        if(i==2) return InpRisk_M15_TP3_Points;
        if(i==3) return InpRisk_M15_TP4_Points;
        return InpRisk_M15_TP5_Points;
    }
    if(InpRiskProfile==RISK_M30_SWING){
        if(i==0) return InpRisk_M30_TP1_Points;
        if(i==1) return InpRisk_M30_TP2_Points;
        if(i==2) return InpRisk_M30_TP3_Points;
        if(i==3) return InpRisk_M30_TP4_Points;
        return InpRisk_M30_TP5_Points;
    }
    if(i==0) return InpRisk_M5_TP1_Points;
    if(i==1) return InpRisk_M5_TP2_Points;
    if(i==2) return InpRisk_M5_TP3_Points;
    if(i==3) return InpRisk_M5_TP4_Points;
    return InpRisk_M5_TP5_Points;
}

void CalcTradeLevels(int dir, double entry_price, double atr, bool tight_mode, double &sl_price, double &tp_prices[]){
    double risk=atr*(tight_mode?InpSoft_SL_Mult:InpSL_Mult);
    if(UseFixedRiskProfile(tight_mode)) risk=RiskProfileSLPoints()*_Point;
    sl_price=(dir==1)?entry_price-risk:entry_price+risk;

    if(UseFixedRiskProfile(tight_mode)){
        for(int i=0;i<5 && i<MAX_TP;i++){
            double dist=RiskProfileTPPoints(i)*_Point;
            tp_prices[i]=(dir==1)?entry_price+dist:entry_price-dist;
        }
        double step=MathAbs(tp_prices[4]-tp_prices[3]);
        if(step<=0) step=MathMax(_Point, RiskProfileTPPoints(0)*_Point);
        for(int i=5;i<MAX_TP;i++) tp_prices[i]=(dir==1)?tp_prices[i-1]+step:tp_prices[i-1]-step;
        return;
    }

    double tp1_dist,tp2_dist;
    if(tight_mode){
        tp1_dist=atr*InpSoft_TP1_Mult;
        tp2_dist=tp1_dist+atr*InpSoft_TP2_Mult;
    } else {
        tp1_dist=atr*InpTP1_Mult;
        tp2_dist=tp1_dist+atr*InpTP1_to_TP2_Mult;
    }
    tp_prices[0]=(dir==1)?entry_price+tp1_dist:entry_price-tp1_dist;
    tp_prices[1]=(dir==1)?entry_price+tp2_dist:entry_price-tp2_dist;
    for(int i=2;i<MAX_TP;i++){
        double sd=(i-1)*atr*InpTP_Step_Mult;
        tp_prices[i]=(dir==1)?tp_prices[1]+sd:tp_prices[1]-sd;
    }
}

string RiskProfileLabel(){
    if(InpRiskProfile==RISK_M15_CONTEXT) return "M15_CONTEXT";
    if(InpRiskProfile==RISK_M30_SWING) return "M30_SWING";
    if(InpRiskProfile==RISK_ATR_DYNAMIC) return "ATR_DYNAMIC";
    return "M5_BALANCED";
}

bool HTFStrongContextOk(int sig, int idx){
    bool checked=false;
    if(InpUseM30Bias){
        int m30=GetHTFDirection(PERIOD_M30, idx);
        checked=true;
        if(m30!=sig) return false;
    }
    if(InpUseM15Setup){
        int m15=GetHTFDirection(PERIOD_M15, idx);
        checked=true;
        if(m15!=sig) return false;
    }
    return checked;
}

ENUM_FILTER_DECISION EvalADXBandAt(int sig, int idx, string &state, string &reason){
    state="OFF"; reason="ADX_OFF"; g_adx_band_value=EMPTY_VALUE;
    if(!InpUseADXBandGuard) return DEC_OK;
    if(h_adx==INVALID_HANDLE){ state="ERR"; reason="ADX_HANDLE"; return DEC_OK; }
    double adx[1];
    if(CopyBuffer(h_adx,0,idx,1,adx)<=0){ state="ERR"; reason="ADX_COPY"; return DEC_OK; }
    g_adx_band_value=adx[0];
    double min_allowed=MathMax(0.0,InpADX_MinAllowed);
    double max_allowed=MathMax(min_allowed,InpADX_MaxAllowed);
    double extreme=MathMax(max_allowed,InpADX_ExtremeLevel);
    if(adx[0] < min_allowed){ state="LOW"; reason="ADX_LOW"; return DEC_BLOCKED; }
    if(adx[0] <= max_allowed){ state="OK"; reason="ADX_OK"; return DEC_OK; }
    if(adx[0] < extreme){ state="LIMITED"; reason="ADX_HIGH_LIMITED"; return DEC_LIMITED; }
    if(HTFStrongContextOk(sig,idx)){ state="EXT_HTF"; reason="ADX_EXTREME_HTF_LIMITED"; return DEC_LIMITED; }
    state="EXTREME"; reason="ADX_EXTREME";
    if(InpADX_ExtremeAction==F_HARD) return DEC_BLOCKED;
    if(InpADX_ExtremeAction==F_SOFT) return DEC_LIMITED;
    return DEC_OK;
}

ENUM_FILTER_DECISION EvalVLineNoiseAt(int sig, int idx, string &state){
    state="OFF";
    if(!InpUseVLineNoiseGuard || InpVLineNoiseAction==F_OFF) return DEC_OK;
    int vdir=0; double vband=0.0; bool noisy=false; string vst="NO_DATA";
    if(!GetVLineDirectionAt(idx,vdir,vst,vband,noisy)){ state="NO_DATA"; return DEC_OK; }
    state=vst+(noisy?" NOISE":" CLEAN");
    if(!noisy) return DEC_OK;
    if(InpVLineNoiseAction==F_HARD) return DEC_BLOCKED;
    return DEC_LIMITED;
}
...
bool EvaluatePipeline(int sig){
    ...
    string adx_state="", adx_reason="";
    ENUM_FILTER_DECISION adx_dec = EvalADXBandAt(sig, 1, adx_state, adx_reason);
    g_adx_band_state=adx_state; g_adx_band_reason=adx_reason;
    string vnoise_state="";
    ENUM_FILTER_DECISION vnoise_dec = EvalVLineNoiseAt(sig, 1, vnoise_state);
    g_vline_noise_state=vnoise_state;
    bool h_atr_block = (atr_dec==DEC_BLOCKED);
    bool s_atr_limited = (atr_dec==DEC_LIMITED);
    bool h_sq = (sq_dec==DEC_BLOCKED);
    bool s_sq = (sq_dec==DEC_LIMITED);
    bool h_adx = (adx_dec==DEC_BLOCKED);
    bool s_adx = (adx_dec==DEC_LIMITED);
    bool h_vnoise = (vnoise_dec==DEC_BLOCKED);
    bool s_vnoise = (vnoise_dec==DEC_LIMITED);
    g_pipe_filter_hard = h_f2 || h_htf || h_est || h_slow_angle || h_sw || h_sideway || h_atr_block || h_sq || h_adx || h_vnoise;
    g_pipe_soft = s_sw || s_atr_limited || s_sq || s_adx || s_vnoise;
...
bool EvaluatePipelineAt(int idx, int sig, bool &out_hard, bool &out_soft){
    ...
    string adx_state="", adx_reason="";
    ENUM_FILTER_DECISION adx_dec = EvalADXBandAt(sig, idx, adx_state, adx_reason);
    if(adx_dec==DEC_BLOCKED) out_hard=true;
    if(adx_dec==DEC_LIMITED) out_soft=true;
    string vnoise_state="";
    ENUM_FILTER_DECISION vnoise_dec = EvalVLineNoiseAt(sig, idx, vnoise_state);
    if(vnoise_dec==DEC_BLOCKED) out_hard=true;
    if(vnoise_dec==DEC_LIMITED) out_soft=true;
    return !out_hard;
}
...
void SetupTrade(int dir, bool tight_mode){
    ...
    CalcTradeLevels(dir,c1,g_atr_val,tight_mode,g_sl_price,g_tp);
    g_sl_initial_dist=MathAbs(c1-g_sl_price);
    for(int i=0;i<MAX_TP;i++)g_tp_hit[i]=false;
    ...
    g_ssl_phase=0;
    // Rulebook fixed profiles and soft mode always force SL ON.
    g_sl_forced=(tight_mode || UseFixedRiskProfile(false));
...
bool IsBREAllowedBlockReason(string reason){
    ...
    return InpBRE_AllowOtherBlocks;
}

string GetBlockReasonAt(int sig, int idx){
    if(InpF2_Action!=F_OFF && EvalF2At(idx)) return "F2";
    ...
    string adx_state="", adx_reason=""; if(EvalADXBandAt(sig,idx,adx_state,adx_reason)==DEC_BLOCKED) return "ADX_BAND";
    string vnoise_state=""; if(EvalVLineNoiseAt(sig,idx,vnoise_state)==DEC_BLOCKED) return "VLINE_NOISE";
    return "FILTER";
}
...
void ArmBlockedRetestReentry(int dir, datetime t, string reason){
    if(!InpUseBlockedRetestReentry || dir==0) return;
    if(!IsBREAllowedBlockReason(reason)) return;
    if(g_last_signal!=0) return;
    int idx=iBarShift(_Symbol,_Period,t);
    if(idx<0) idx=1;
    double atr=g_atr_val;
    if(atr<=0){ double a[1]; if(CopyBuffer(h_atr,0,idx,1,a)>0) atr=a[0]; }
    if(atr<=0) atr=_Point*100;
    g_bre_armed=true;
    g_bre_pullback_seen=false;
    g_bre_dir=dir;
    g_bre_counter=MathMax(1,InpBRE_Bars);
    g_bre_time=t;
    g_bre_atr=atr;
    g_bre_reason=reason;
    Print("[BRE-ARM] ",(dir==1?"BUY":"SELL")," reason=",reason," bars=",g_bre_counter," atr=",DoubleToString(atr,_Digits));
}

void CancelBlockedRetestReentry(string why){
    if(g_bre_armed) Print("[BRE-CANCEL] ",(g_bre_dir==1?"BUY":"SELL")," ",why);
    g_bre_armed=false; g_bre_pullback_seen=false; g_bre_dir=0; g_bre_counter=0; g_bre_time=0; g_bre_atr=0.0; g_bre_reason="OFF";
}

void FireBlockedRetestReentry(int dir, datetime t, double price){
    string bre_fire_reason=g_bre_reason;
    Print("[BRE-FIRE] ",(dir==1?"BUY":"SELL")," after=",bre_fire_reason," price=",DoubleToString(price,_Digits));
    SetupTrade(dir,InpBRE_EnterAsLimited);
    EAOpen(dir);
    if(InpIndicatorOnly || g_last_signal!=0){
        DrawSignalLabel(dir,t,price,SC_REENTRY);
        ColorSignalCandle(dir,t);
        DrawTPSLLines();
    }
    JournalSignalEvent(dir,t,price,SC_REENTRY,"",bre_fire_reason,true,"",InpBRE_EnterAsLimited?"BRE_LIMITED":"BRE_VALID");
    CancelBlockedRetestReentry("FIRED");
}

void ProcessBlockedRetestReentry(){
    if(!InpUseBlockedRetestReentry || !g_bre_armed || g_last_signal!=0) return;
    if(g_bre_counter<=0){ CancelBlockedRetestReentry("EXPIRED"); return; }
    g_bre_counter--;

    int idx=1;
    double ef[1], sm[1], atr_arr[1];
    if(CopyBuffer(h_ema_fast,0,idx,1,ef)<=0 || CopyBuffer(h_sma_slow,0,idx,1,sm)<=0 || CopyBuffer(h_atr,0,idx,1,atr_arr)<=0) return;
    double atr=MathMax(atr_arr[0],_Point);
    double o=iOpen(_Symbol,_Period,idx), h=iHigh(_Symbol,_Period,idx), l=iLow(_Symbol,_Period,idx), c=iClose(_Symbol,_Period,idx);

    bool ma_aligned=(g_bre_dir==1 ? ef[0]>sm[0] : ef[0]<sm[0]);
    bool ma_opposite=(g_bre_dir==1 ? ef[0]<sm[0] : ef[0]>sm[0]);
    if(InpBRE_RequireMAAlign && ma_opposite){ CancelBlockedRetestReentry("MA_OPPOSITE"); return; }

    double top=MathMax(ef[0],sm[0]), bot=MathMin(ef[0],sm[0]);
    double buffer=InpBRE_RetestBufferATR*atr;
    bool touched=(g_bre_dir==1 ? l<=top+buffer : h>=bot-buffer);
    if(touched) g_bre_pullback_seen=true;
    if(!g_bre_pullback_seen) return;
...
bool FindBlockedRetestReentryAt(int block_idx, int dir, string reason, datetime &rt, double &rp){
    rt=0; rp=0;
    if(!InpUseBlockedRetestReentry || dir==0) return false;
    if(!IsBREAllowedBlockReason(reason)) return false;
    int bars=InpBRE_Bars; if(bars<1) bars=1;
    int last_idx=block_idx-bars; if(last_idx<1) last_idx=1;
    bool pullback_seen=false;

    for(int b=block_idx-1; b>=last_idx; b--){
        double ef[1], sm[1], atr_arr[1];
        if(CopyBuffer(h_ema_fast,0,b,1,ef)<=0 || CopyBuffer(h_sma_slow,0,b,1,sm)<=0 || CopyBuffer(h_atr,0,b,1,atr_arr)<=0) continue;
        double atr=MathMax(atr_arr[0],_Point);
        double o=iOpen(_Symbol,_Period,b), h=iHigh(_Symbol,_Period,b), l=iLow(_Symbol,_Period,b), c=iClose(_Symbol,_Period,b);

        bool ma_aligned=(dir==1 ? ef[0]>sm[0] : ef[0]<sm[0]);
        bool ma_opposite=(dir==1 ? ef[0]<sm[0] : ef[0]>sm[0]);
        if(InpBRE_RequireMAAlign && ma_opposite) return false;

        double top=MathMax(ef[0],sm[0]), bot=MathMin(ef[0],sm[0]);
        double buffer=InpBRE_RetestBufferATR*atr;
        bool touched=(dir==1 ? l<=top+buffer : h>=bot-buffer);
        if(touched) pullback_seen=true;
        if(!pullback_seen) continue;
        ...
        for(int attempt=1;attempt<=max_r;attempt++){
            ...
            if(sent&&(rs.retcode==TRADE_RETCODE_DONE||rs.retcode==TRADE_RETCODE_DONE_PARTIAL)){
                ...
                if(!PositionSelectByTicket(tk)){
                    ...
                    double close_px=(rs.price>0.0?rs.price:r.price);
                    JournalMarkPositionExit(reason,close_px);
...
void DrawSignalLabel(int dir,datetime t,double price,ENUM_SIGNAL_CLASS sc){
    ...
    switch(sc){
        ...
        case SC_REENTRY: txt=(dir==1?"RE-ENTRY BUY":"RE-ENTRY SELL"); clr=DB_CLR_CREAM; break;
    ...
    if(ObjectFind(0,nm)>=0){
        ...
        // v13.11.44 — do not let stale BLOCKED labels survive a settings change.
...
void UpdateVisualForSignal(int dir,double entry_price,double atr,bool tight_mode){
    ...
    CalcTradeLevels(dir,entry_price,atr,tight_mode,g_visual_sl,g_visual_tp);
...
void DrawF3RecoveryTPSL(int dir, datetime rt, double rp, double atr){
    ...
    double f3_tp[MAX_TP];
    double sl_px=0.0;
    CalcTradeLevels(dir,rp,atr,soft,sl_px,f3_tp);
    double tp1_px = f3_tp[0];
    double tp2_px = f3_tp[1];
...
void ScanHistory(){
    ...
    int sc=0,sc_blocked=0,sc_soft=0,sc_reentry=0;
    ...
    for(int i=scan;i>=1;i--){
        ...
        if(hard){
            ...
            datetime bre_t=0; double bre_p=0;
            string bre_reason=GetBlockReasonAt(sig,i);
            if(FindBlockedRetestReentryAt(i,sig,bre_reason,bre_t,bre_p)){
                DrawSignalLabel(sig,bre_t,bre_p,SC_REENTRY);
                ColorSignalCandle(sig,bre_t);
                sc_reentry++;
                bool bre_limited=InpBRE_EnterAsLimited;
                CalcTradeLevels(sig,bre_p,ah[0],bre_limited,vis_sl,vis_tp);
                for(int t=0;t<MAX_TP;t++)vis_hit[t]=false;
                vis_unlock=5;
                has_last=true;last_dir=sig;last_entry=bre_p;last_atr=ah[0];last_cls=SC_REENTRY;
                bre_found=true;
            }
        }
        ...
        if(sc_class!=SC_BLOCKED){
            CalcTradeLevels(sig,sp,ah[0],last_tight,vis_sl,vis_tp);
            for(int t=0;t<MAX_TP;t++)vis_hit[t]=false;
    ...
    Print("[SCAN] ",sc," crossovers | VALID=",(sc-sc_blocked-sc_soft)," LIMITED=",sc_soft," BLOCKED=",sc_blocked," REENTRY=",sc_reentry);
...
void DrawDashboard(bool force){
    ...
    int nr=33,ph=(nr+1)*DB_LINE_H+DB_Y_BASE+8;
    ...
    int dash_sig=(g_last_signal!=0 ? g_last_signal : (g_ema_fast>=g_sma_slow ? 1 : -1));
    ...
    string adx_state="", adx_reason=""; ENUM_FILTER_DECISION adx_dec=EvalADXBandAt(dash_sig,1,adx_state,adx_reason);
    string vnoise_state=""; ENUM_FILTER_DECISION vnoise_dec=EvalVLineNoiseAt(dash_sig,1,vnoise_state);
    if(htf_block) n_hard++;
    if(est_block) n_hard++;
    ...
    if(adx_dec==DEC_BLOCKED) n_hard++;
    if(vnoise_dec==DEC_BLOCKED) n_hard++;
    ...
    DrawDR("BREASON",row++,"Block Reason",g_last_block_reason,(g_last_block_reason=="---"?DB_CLR_GRAY:DB_CLR_ORANGE),yb);

    DrawFilterRow("F2",row++,"F2 CrossDist",InpF2_Action,g_f2_trig,g_f2_trig?"FAR":"OK",yb);

    string htfv = StringFormat("M30=%s M15=%s RevOVR=%s", InpUseM30Bias?"ON":"OFF", InpUseM15Setup?"ON":"OFF", InpHTF_AllowReversalOverride?"ON":"OFF");
    ...
    DrawDR("EST",row++,"V-Line Hard", (InpUseVLineGuard?(g_est_state+" "+EnumToString(InpVLine_TF)):"OFF"), !InpUseVLineGuard?DB_CLR_GRAY:(est_block?DB_CLR_RED:(g_est_dir>0?DB_CLR_GREEN:DB_CLR_ORANGE)), yb);
    DrawDR("VLNOISE",row++,"V-Line Noise", InpUseVLineNoiseGuard?vnoise_state:"OFF", !InpUseVLineNoiseGuard?DB_CLR_GRAY:(vnoise_dec==DEC_BLOCKED?DB_CLR_RED:(vnoise_dec==DEC_LIMITED?DB_CLR_YELLOW:DB_CLR_GREEN)), yb);
    DrawDR("ADXBG",row++,"ADX Band", InpUseADXBandGuard?(adx_state+" "+DoubleToString(g_adx_band_value,1)):"OFF", !InpUseADXBandGuard?DB_CLR_GRAY:(adx_dec==DEC_BLOCKED?DB_CLR_RED:(adx_dec==DEC_LIMITED?DB_CLR_YELLOW:DB_CLR_GREEN)), yb);
    DrawDR("RISK",row++,"Risk Profile",RiskProfileLabel(),DB_CLR_CYAN,yb);
    ...
    if(nb){
        ...
        if(sig!=0){
            ...
            if(!PreCheckMarketConditions()){
                ...
                JournalSignalEvent(sig,st,sp,SC_BLOCKED,reason,"",false,"MARKET","LIVE");
            } else {
                ...
                if(pipe_ok&&!g_pipe_hard){
                    ...
                    bool will_execute=(g_last_signal==0);
                    JournalSignalEvent(sig,st,sp,live_label,"","",will_execute,will_execute?"":"POSITION_ACTIVE","LIVE");
                    if(will_execute){
                    ...
                    g_last_decision="B";
                    g_last_block_reason=GetBlockReasonAt(sig,1);
                    JournalSignalEvent(sig,st,sp,live_label,g_last_block_reason,"",false,"BLOCKED","LIVE");
                    DrawSignalLabel(sig,st,sp,live_label);
                    if(ShouldShowSignalVisual(live_label)) ColorSignalCandle(sig,st);
                    if(g_last_signal==0){
                        ClearVisualTradeState();
                        ...
                        string breason=g_last_block_reason;
                        ArmF3Recovery(sig,st,breason);
                        ArmBlockedRetestReentry(sig,st,breason);
            ...
            if(!aligned){
                ...
                if(!pipe_ok||g_pipe_hard){
                    ...
                    if(momentum_ok){
                        ...
                        datetime rt=iTime(_Symbol,_Period,1);
                        double rp=iClose(_Symbol,_Period,1);
                        SetupTrade(rdir,true);EAOpen(rdir);
                        if(g_last_signal!=0){
                            DrawSignalLabel(rdir,rt,rp,SC_REENTRY);
                            DrawTPSLLines();
                        }
                        JournalSignalEvent(rdir,rt,rp,SC_REENTRY,"",g_reentry_reason,true,"","CLASSIC_REENTRY");
                        g_awaiting_reentry=false;g_reentry_dir=0;g_reentry_reason="";
                    }
                }
            }
