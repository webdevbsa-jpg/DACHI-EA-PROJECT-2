// Dachi_Trader_v13_11_40.mq5 — Expert Advisor
// Version: 13.11.40
// Clean Core branch: simplified runtime pipeline (F2 + HTF gate).

#property copyright   "Daniel @danieljulyanto"
#property version     "13.11.40"
#property description "Dachi Trader v13.11.40 — Expert Advisor"
...
#define DB_CLR_CREAM      C'245,222,179'  // v13.11.40: RE-ENTRY label color
...
bool LIC_VerifyOnce(){
    ...
    string body = StringFormat("{\"account_number\":\"%I64u\",\"ea_version\":\"13.11.40\"}", (ulong)account);
...
int CleanupAllEAObjects(){
    // v13.11.40: delete EA objects by prefix + object type on the main chart.
...
int OnInit(){
    ...
    // v13.11.40 — startup self-clean catches remnants from a prior abnormal
    ...
    Print("==============================================================");
    Print("[INIT] Dachi Trader v13.11.40 | ",_Symbol," ",EnumToString(_Period),
...
void OnDeinit(const int reason){
    ...
    Print("[DEINIT] Dachi v13.11.40 removed reason=",reason," cleanup_deleted=",deleted);
...
void JournalSignalEvent(int dir, datetime t, double price, ENUM_SIGNAL_CLASS sc, string block_reason, string bre_reason, bool executed, string skip_reason, string origin){
    ...
    double vband=0.0; bool noisy=false; int vdir=GetVLineDirectionAt(idx,vband,noisy);
    ...
    g_journal_pending_suffix=(executed?"YES":"NO")+StringFormat(";%s;%s;%s;",JClean(skip_reason),JClean(block_reason),JClean(bre_reason))+
        ...
        JNum((ema-slow)/_Point,1)+";"+JNum(angle,1)+";"+JClean(g_est_state)+";"+DirName(vdir)+";"+JClean(sw_state)+";"+JNum(sw,2)+";"+
...
int ComputeFilterHash(){
    ...
    h = (h ^ (uint)0x13C00400)             * 16777619;   // logic version marker
    return (int)h;
}
...
void DrawSignalLabel(int dir,datetime t,double price,ENUM_SIGNAL_CLASS sc){
    ...
    if(ObjectFind(0,nm)>=0){
        ...
        // v13.11.40 — do not let stale BLOCKED labels survive a settings change.
