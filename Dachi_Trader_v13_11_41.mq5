// Dachi_Trader_v13_11_41.mq5 — Expert Advisor
// Version: 13.11.41
// Clean Core branch: simplified runtime pipeline (F2 + HTF gate).

#property copyright   "Daniel @danieljulyanto"
#property version     "13.11.41"
#property description "Dachi Trader v13.11.41 — Expert Advisor"
...
#define DB_CLR_CREAM      C'245,222,179'  // v13.11.41: RE-ENTRY label color
...
input bool   InpJournalUseCommonFolder = false; // write to MQL5/Files/Common when true
...
bool LIC_VerifyOnce(){
    ...
    string body = StringFormat("{\"account_number\":\"%I64u\",\"ea_version\":\"13.11.41\"}", (ulong)account);
...
int CleanupAllEAObjects(){
    // v13.11.41: delete EA objects by prefix + object type on the main chart.
...
int OnInit(){
    ...
    // v13.11.41 — startup self-clean catches remnants from a prior abnormal
    ...
    Print("==============================================================");
    Print("[INIT] Dachi Trader v13.11.41 | ",_Symbol," ",EnumToString(_Period),
...
void OnDeinit(const int reason){
    ...
    Print("[DEINIT] Dachi v13.11.41 removed reason=",reason," cleanup_deleted=",deleted);
...
void JournalAppendLine(string line){
    ...
    string path=JournalSignalPath();
    int flags=FILE_READ|FILE_WRITE|FILE_TXT|FILE_ANSI;
    if(InpJournalUseCommonFolder) flags|=FILE_COMMON;
    int h=FileOpen(path,flags);
    if(h==INVALID_HANDLE){ Print("[JOURNAL] FileOpen failed: ",path," err=",GetLastError()); return; }
    bool need_header=(FileSize(h)==0);
    FileSeek(h,0,SEEK_END);
    if(need_header){
        FileWriteString(h,"event_time;symbol;timeframe;origin;signal_class;direction;signal_entry_price;signal_exit_time;signal_exit_price;signal_move_pts;signal_duration_bars;signal_result;signal_exit_reason;executed;skip_reason;block_reason;bre_reason;atr;spread;ema_fast;slow_ma;ma_gap_pts;slow_angle_deg;vline_state;vline_dir;sw_state;sw_score;adx;di_plus;di_minus;sl;tp1;tp2;tp3;tp4;tp5\r\n");
    }
...
int ComputeFilterHash(){
    ...
    h = (h ^ (uint)0x13C00410)             * 16777619;   // logic version marker
    return (int)h;
}
...
void DrawSignalLabel(int dir,datetime t,double price,ENUM_SIGNAL_CLASS sc){
    ...
    if(ObjectFind(0,nm)>=0){
        ...
        // v13.11.41 — do not let stale BLOCKED labels survive a settings change.
