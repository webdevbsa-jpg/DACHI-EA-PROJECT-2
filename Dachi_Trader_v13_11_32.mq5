// Dachi_Trader_v13_11_32.mq5 — Expert Advisor
// Version: 13.11.32
// Clean Core branch: simplified runtime pipeline (F2 + HTF gate).

#property copyright   "Daniel @danieljulyanto"
#property version     "13.11.32"
#property description "Dachi Trader v13.11.32 — Expert Advisor"
...
input double             InpECI_ChopThreshold  = 0.72;   // >= threshold means random/choppy
input double             InpECI_MixedThreshold = 0.58;   // dashboard MIXED/ORDERED boundary
...
bool LIC_VerifyOnce(){
    ...
    string body = StringFormat("{\"account_number\":\"%I64u\",\"ea_version\":\"13.11.32\"}", (ulong)account);
...
int OnInit(){
    ...
    Print("==============================================================");
    Print("[INIT] Dachi Trader v13.11.32 | ",_Symbol," ",EnumToString(_Period),
...
void OnDeinit(const int reason){
    ...
    LIC_ClearOverlay();
    Print("[DEINIT] Dachi v13.11.32 removed");
}
...
uint ComputeFilterHash(){
    ...
    h = (h ^ (uint)0x13C00320)             * 16777619;   // logic version marker
    return h;
}
...
bool CalcECIAt(int idx, double &score, string &state){
    ...
    int total=0, noisy_count=0, dir_changes=0;
    int prev_dir=0;
    double path=0.0;
    double newest_close=iClose(_Symbol,_Period,idx);
    double oldest_close=iClose(_Symbol,_Period,idx+look-1);
    ...
    for(int i=idx; i<idx+look; i++){
        ...
        if(bucket==2 || bucket==3 || bucket==4) noisy_count++;
        int cur_dir=(c>o?1:(c<o?-1:0));
        if(cur_dir!=0){
            if(prev_dir!=0 && cur_dir!=prev_dir) dir_changes++;
            prev_dir=cur_dir;
        }
        if(i<idx+look-1){
            double next_c=iClose(_Symbol,_Period,i+1);
            path += MathAbs(c-next_c);
        }
    ...
    double entropy_norm = entropy / MathLog(7.0);
    double efficiency = (path>_Point*0.1 ? MathMin(1.0, MathAbs(newest_close-oldest_close)/path) : 0.0);
    double noisy_ratio = (double)noisy_count/(double)total;
    double alternation = (total>1 ? (double)dir_changes/(double)(total-1) : 0.0);

    // v13.11.32: entropy alone was too strict on M1/M5 because a healthy trend
    // can still contain mixed candle shapes. The chop score now requires entropy
    // plus weak directional efficiency/noisy candle structure, so ordered trends
    // are not LIMITED just because candle anatomy is diverse.
    score = entropy_norm*0.45 + (1.0-efficiency)*0.35 + noisy_ratio*0.15 + alternation*0.05;
    score = MathMax(0.0, MathMin(1.0, score));

    string detail=DoubleToString(score,2)+" E"+DoubleToString(efficiency,2);
    if(score>=InpECI_ChopThreshold) state="CHOP "+detail;
    else if(score>=InpECI_MixedThreshold) state="MIXED "+detail;
    else state="ORDER "+detail;
    return true;
}
...
void DrawSignalLabel(int dir,datetime t,double price,ENUM_SIGNAL_CLASS sc){
    ...
    if(ObjectFind(0,nm)>=0){
        ...
        // v13.11.32 — do not let stale BLOCKED labels survive a settings change.
