// Dachi_Trader_v13_11_23.mq5 — Expert Advisor
// Version: 13.11.23
// Clean Core branch: simplified runtime pipeline (F2 + HTF gate).

#property copyright   "Daniel @danieljulyanto"
#property version     "13.11.23"
#property description "Dachi Trader v13.11.23 — Expert Advisor"
...
input int             InpEST_VisualBars         = 500;
...
bool LIC_VerifyOnce(){
    ...
    string body = StringFormat("{\"account_number\":\"%I64u\",\"ea_version\":\"13.11.23\"}", (ulong)account);
...
int OnInit(){
    ...
    Print("==============================================================");
    Print("[INIT] Dachi Trader v13.11.23 | ",_Symbol," ",EnumToString(_Period),
...
void OnDeinit(const int reason){
    ...
    LIC_ClearOverlay();
    Print("[DEINIT] Dachi v13.11.23 removed");
}
...
uint ComputeFilterHash(){
    ...
    h = (h ^ (uint)0x13C00230)             * 16777619;   // logic version marker
    return h;
}
...
void DrawEvasiveSTVisual(){
    ...
    ENUM_TIMEFRAMES tf=(InpEST_TF==PERIOD_CURRENT?_Period:InpEST_TF);
    int bars=InpEST_VisualBars;
    int visible=(int)ChartGetInteger(0,CHART_VISIBLE_BARS);
    int chart_sec=PeriodSeconds(_Period), st_sec=PeriodSeconds(tf);
    if(visible>0 && chart_sec>0 && st_sec>0){
        int needed=(int)MathCeil((double)visible*(double)chart_sec/(double)st_sec)+50;
        bars=MathMax(bars,needed);
    }
    bars=MathMax(5,MathMin(bars,1200));
    if(Bars(_Symbol,tf)<bars+InpEST_ATRLength+20) bars=MathMax(5,Bars(_Symbol,tf)-InpEST_ATRLength-25);
    if(bars<5) return;
