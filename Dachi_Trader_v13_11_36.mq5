// Dachi_Trader_v13_11_36.mq5 — Expert Advisor
// Version: 13.11.36
// Clean Core branch: simplified runtime pipeline (F2 + HTF gate).

#property copyright   "Daniel @danieljulyanto"
#property version     "13.11.36"
#property description "Dachi Trader v13.11.36 — Expert Advisor"
...
#define DB_CLR_CREAM      C'245,222,179'  // v13.11.36: RE-ENTRY label color
...
input bool   InpBRE_AllowECIBlocks        = false;  // default false: ECI/CHOP blocks are sideway-risky
...
bool LIC_VerifyOnce(){
    ...
    string body = StringFormat("{\"account_number\":\"%I64u\",\"ea_version\":\"13.11.36\"}", (ulong)account);
...
int CleanupAllEAObjects(){
    // v13.11.36: delete EA objects by prefix + object type on the main chart.
...
int OnInit(){
    ...
    // v13.11.36 — startup self-clean catches remnants from a prior abnormal
    ...
    Print("==============================================================");
    Print("[INIT] Dachi Trader v13.11.36 | ",_Symbol," ",EnumToString(_Period),
...
void OnDeinit(const int reason){
    ...
    Print("[DEINIT] Dachi v13.11.36 removed reason=",reason," cleanup_deleted=",deleted);
...
uint ComputeFilterHash(){
    ...
    h = (h ^ (uint)(InpBRE_AllowECIBlocks?1:0)) * 16777619;
    ...
    h = (h ^ (uint)0x13C00360)             * 16777619;   // logic version marker
    return h;
}
...
bool IsBREAllowedBlockReason(string reason){
    ...
    if(reason=="ECI") return InpBRE_AllowECIBlocks;
...
string GetBlockReasonAt(int sig, int idx){
    ...
    if(ECITriggered(idx) && InpECI_Action==F_HARD) return "ECI";
...
void DrawSignalLabel(int dir,datetime t,double price,ENUM_SIGNAL_CLASS sc){
    ...
    if(ObjectFind(0,nm)>=0){
        ...
        // v13.11.36 — do not let stale BLOCKED labels survive a settings change.
...
void DrawDashboard(bool force){
    ...
    int nr=29,ph=(nr+1)*DB_LINE_H+DB_Y_BASE+8;
            ...
            if(!PreCheckMarketConditions()){
                ...
                if(pipe_ok&&!g_pipe_hard){
                    ...
                    if(g_last_signal==0){
                        ...
                        string breason=GetBlockReasonAt(sig,1);
