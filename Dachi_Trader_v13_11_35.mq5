// Dachi_Trader_v13_11_35.mq5 — Expert Advisor
// Version: 13.11.35
// Clean Core branch: simplified runtime pipeline (F2 + HTF gate).

#property copyright   "Daniel @danieljulyanto"
#property version     "13.11.35"
#property description "Dachi Trader v13.11.35 — Expert Advisor"
...
#define DB_CLR_CREAM      C'245,222,179'  // v13.11.35: RE-ENTRY label color
...
bool LIC_VerifyOnce(){
    ...
    string body = StringFormat("{\"account_number\":\"%I64u\",\"ea_version\":\"13.11.35\"}", (ulong)account);
...
int CleanupAllEAObjects(){
    // v13.11.35: delete EA objects by prefix + object type on the main chart.
...
int OnInit(){
    ...
    // v13.11.35 — startup self-clean catches remnants from a prior abnormal
    ...
    Print("==============================================================");
    Print("[INIT] Dachi Trader v13.11.35 | ",_Symbol," ",EnumToString(_Period),
...
void OnDeinit(const int reason){
    ...
    Print("[DEINIT] Dachi v13.11.35 removed reason=",reason," cleanup_deleted=",deleted);
...
uint ComputeFilterHash(){
    ...
    h = (h ^ (uint)0x13C00350)             * 16777619;   // logic version marker
    return h;
}
...
bool BREDIOk(int dir, int idx){
    ...
    double dp[1], dm[1];
    if(CopyBuffer(h_adx,1,idx,1,dp)<=0 || CopyBuffer(h_adx,2,idx,1,dm)<=0) return false;
...
bool FindBlockedRetestReentryAt(int block_idx, int dir, datetime &rt, double &rp){
    rt=0; rp=0;
...
void DrawSignalLabel(int dir,datetime t,double price,ENUM_SIGNAL_CLASS sc){
    ...
    if(ObjectFind(0,nm)>=0){
        ...
        // v13.11.35 — do not let stale BLOCKED labels survive a settings change.
...
void ScanHistory(){
    ...
    for(int i=scan;i>=1;i--){
        ...
        if(hard){
            ...
            if(FindBlockedRetestReentryAt(i,sig,bre_t,bre_p)){
